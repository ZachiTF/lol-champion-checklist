...
function detectBenchRow(buf, W, H) {
  ...
  const xa = Math.round(W * 0.24),
    xb = Math.round(W * 0.75);
  ...
  for (let i = 0; i < nSlots; i++)
    slots.push({ cx: left + pitch * (i + 0.5), cy });
  return { slots, bandH };
}
...
function detectTeamCircles(buf, W, H) {
  const xa = Math.round(W * 0.035),
    xb = Math.round(W * 0.095);
  ...
  let cxNum = 0,
    cxDen = 0;
  const cxa = Math.round(W * 0.03),
    cxb = Math.round(W * 0.1);
